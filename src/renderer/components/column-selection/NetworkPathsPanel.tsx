...
const NetworkPathsPanel: React.FC<NetworkPathsPanelProps> = ({
  ...
  const handlePdfPathChange = useCallback((value: string) => {
    if (!validateNetworkPath(value)) {
      return; // Validation failed, don't update
    }
  ...
  return (
    <div className="config-section">
      ...
      <p className="text-muted mb-3">
        Configure network paths for CSV logging. These paths will be recorded in download reports 
        but are separate from your local download folders above.
      </p>
      ...
      <div className="alert alert-info">
        <strong>Note:</strong> Network paths are used for generating CSV reports that reference files 
        on shared network locations. These are different from your local download folders and should 
        point to network-accessible locations where other users can access the downloaded files.
      </div>
    </div>
  );
};
