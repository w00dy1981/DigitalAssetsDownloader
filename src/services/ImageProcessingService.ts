...
try {
  ...
  logger.warn('Sharp not available, image processing disabled', 'ImageProcessingService', { 
...
export class ImageProcessingService {
  ...
  public async getImageMetadata(imageBuffer: Buffer): Promise<ImageMetadata> {
    if (!this.isSharpAvailable || !sharp) {
      logger.warn('Sharp not available - cannot get image metadata', 'ImageProcessingService');
      return {};
    }
    ...
    try {
      ...
      const processedError = errorHandler.handleError(error, 'ImageProcessingService', { throwOnError: false });
      logger.warn('Error getting image metadata', 'ImageProcessingService', { error: processedError.message });
      return {};
    }
  }
   ...
   */
  public async needsBackgroundProcessing(imageBuffer: Buffer): Promise<boolean> {
    if (!this.isSharpAvailable || !sharp) {
      return false;
    }
    ...
    try {
      ...
      if (metadata.format === 'png' && metadata.hasAlpha) {
        ...
        for (let i = 3; i < data.length; i += 40) { // Sample every 10th pixel's alpha
          if (data[i] < 255) {
            logger.info('Found transparency in PNG - needs background processing', 'ImageProcessingService');
            return true;
          }
        }
        
        logger.info('PNG has alpha channel but no actual transparency - skipping processing', 'ImageProcessingService');
        return false;
      }
      ...
      const processedError = errorHandler.handleError(error, 'ImageProcessingService', { throwOnError: false });
      logger.warn('Error checking image for background processing needs', 'ImageProcessingService', { 
  ...
  public async convertToJpeg(
    ...
    if (!this.isSharpAvailable || !sharp) {
      logger.warn('Sharp not available - returning original image buffer', 'ImageProcessingService');
    ...
    try {
      const needsProcessing = backgroundProcessingEnabled && 
      ...
      if (needsProcessing) {
        ...
        logger.info('Image processed: transparency replaced with white background', 'ImageProcessingService');
        return { 
      ...
      const processedError = errorHandler.handleError(error, 'ImageProcessingService', { throwOnError: false });
      logger.error('Sharp image processing failed', processedError, 'ImageProcessingService');
  ...
  public async processImage(
    ...
    try {
      ...
      const processedError = errorHandler.handleError(error, 'ImageProcessingService', { throwOnError: false });
      logger.error('Image processing failed', processedError, 'ImageProcessingService');
  ...
  public async getImageDimensions(buffer: Buffer): Promise<{width: number, height: number} | null> {
    ...
    try {
      ...
      const processedError = errorHandler.handleError(error, 'ImageProcessingService', { throwOnError: false });
      logger.warn('Error getting image dimensions', 'ImageProcessingService', { error: processedError.message });
      return null;
    }
  }
}
