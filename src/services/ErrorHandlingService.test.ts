...
describe('ErrorHandlingService', () => {
  ...
  describe('Error Classes', () => {
    test('ValidationError should include field details', () => {
      ...
      expect(error.message).toBe('Validation failed for partNoColumn: is required');
      expect(error.code).toBe('VALIDATION_ERROR');
      expect(error.context).toBe('Validation');
    });

    test('ConfigurationError should include setting details', () => {
      const error = new ConfigurationError('imageFolder', 'folder does not exist');
      
      expect(error.setting).toBe('imageFolder');
      expect(error.message).toBe('Configuration error for imageFolder: folder does not exist');
      expect(error.code).toBe('CONFIG_ERROR');
    });

    test('DownloadError should include URL and HTTP status', () => {
      const error = new DownloadError('https://example.com/image.jpg', 'Network timeout', 504, 2);
    ...
    test('FileSystemError should include operation and path', () => {
      const error = new FileSystemError('write', '/tmp/test.jpg', 'Permission denied');
  ...
  describe('handleError', () => {
    test('should handle BaseApplicationError instances', () => {
      ...
      const result = service.handleError(originalError, 'TestContext', { throwOnError: false });
    ...
    test('should convert PathSecurityError to FileSystemError', () => {
      const originalError = new PathSecurityError('Path traversal detected', '../../../etc/passwd');
    ...
    test('should categorize timeout errors as NetworkError', () => {
      const originalError = new Error('ECONNABORTED: timeout of 30000ms exceeded');
    ...
    test('should not throw when throwOnError is false', () => {
      ...
      const result = service.handleError(error, 'TestContext', { throwOnError: false });
  ...
  describe('withRetry', () => {
    ...
    test('should retry on retryable errors', async () => {
      ...
      const result = await service.withRetry(operation, 'TestOperation', { maxAttempts: 2 });
    ...
    test('should not retry on non-retryable errors', async () => {
      const operation = jest.fn().mockRejectedValue(new ValidationError('field', 'value', 'invalid'));
    ...
    test('should fail after max attempts', async () => {
      const operation = jest.fn().mockRejectedValue(new NetworkError('request', 'timeout'));
  ...
  describe('wrapAsync', () => {
    test('should execute function successfully', async () => {
      ...
      const wrapped = service.wrapAsync(fn, 'TestContext', { throwOnError: true });
  ...
  describe('formatErrorForUser', () => {
    ...
    test('should format download errors based on HTTP status', () => {
      const notFoundError = new DownloadError('https://example.com/image.jpg', 'Not found', 404);
      expect(service.formatErrorForUser(notFoundError)).toBe('File not found. Please check the URL.');
      
      const serverError = new DownloadError('https://example.com/image.jpg', 'Server error', 500);
      expect(service.formatErrorForUser(serverError)).toBe('Server error. Please try again later.');
      
      const genericError = new DownloadError('https://example.com/image.jpg', 'Network error');
      expect(service.formatErrorForUser(genericError)).toBe('Download failed. Please check your internet connection.');
    });

    test('should format file system errors appropriately', () => {
      const permissionError = new FileSystemError('write', '/tmp/test.jpg', 'Permission denied');
      expect(service.formatErrorForUser(permissionError)).toBe('Permission denied. Please check file permissions.');
      
      const notFoundError = new FileSystemError('read', '/tmp/missing.jpg', 'File not found');
      expect(service.formatErrorForUser(notFoundError)).toBe('File or folder not found. Please check the path.');
      
      const existsError = new FileSystemError('create', '/tmp/existing.jpg', 'File already exists');
      expect(service.formatErrorForUser(existsError)).toBe('File already exists. Please choose a different location.');
    });
    ...
    test('should format network errors generically', () => {
      ...
      expect(formatted).toBe('Network error. Please check your internet connection.');
    });

    test('should provide generic message for unknown errors', () => {
      const unknownError = new BaseApplicationError('Unknown issue', 'Unknown', 'UNKNOWN');
  ...
  describe('safeFileOperation', () => {
    test('should execute operation successfully', async () => {
      ...
      const result = await service.safeFileOperation(operation, 'read', '/tmp/test.txt');
    ...
    test('should convert PathSecurityError to FileSystemError', async () => {
      const operation = jest.fn().mockRejectedValue(
        new PathSecurityError('Path traversal detected', '../../../etc/passwd')
      );
  ...
  describe('getErrorStats', () => {
    ...
    test('should track error occurrences', () => {
      ...
      try {
        service.handleError(new ValidationError('field1', 'value', 'invalid'), 'Test');
      } catch (e) {}
      
      try {
        service.handleError(new ValidationError('field2', 'value', 'invalid'), 'Test');
      } catch (e) {}
