...
describe('ValidationService', () => {
  ...
  describe('validateRequired', () => {
    ...
    it('should allow empty values when allowEmpty is true', () => {
      const result = ValidationService.validateRequired('', 'Field', { allowEmpty: true });
  ...
  describe('validateStringLength', () => {
    it('should validate string length correctly', () => {
      // Valid length
      const result = ValidationService.validateStringLength('test', 'Field', 2, 10);
      expect(result.isValid).toBe(true);
    });
    
    it('should reject non-string values', () => {
      const result = ValidationService.validateStringLength(123 as any, 'Field');
      expect(result.isValid).toBe(false);
    ...
    it('should validate minimum length', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Field must be at least 5 characters long');
    });
    
    it('should validate maximum length', () => {
      const result = ValidationService.validateStringLength('toolong', 'Field', undefined, 3);
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Field must be no more than 3 characters long');
    });
    
    it('should trim whitespace before validation', () => {
      const result = ValidationService.validateStringLength('  ab  ', 'Field', 2, 2);
      expect(result.isValid).toBe(true);
    });
  });
  ...
  describe('validateNumberRange', () => {
    ...
    it('should reject non-numeric values', () => {
      ...
      invalidValues.forEach(value => {
        const result = ValidationService.validateNumberRange(value as any, 'Field');
        expect(result.isValid).toBe(false);
    ...
    it('should validate maximum value', () => {
      const result = ValidationService.validateNumberRange(15, 'Field', undefined, 10);
      expect(result.isValid).toBe(false);
  ...
  describe('validateEmail', () => {
    ...
    it('should reject invalid email addresses', () => {
      ...
      const result = ValidationService.validateEmail(borderlineCase);
  ...
  describe('validateFilePath', () => {
    ...
    it('should validate safe file paths', () => {
      const result = ValidationService.validateFilePath('/valid/path/file.txt', 'File Path');
      expect(result.isValid).toBe(true);
      expect(mockIsPathSafe).toHaveBeenCalledWith('/valid/path/file.txt', undefined);
    });
    
    it('should reject unsafe file paths', () => {
      mockIsPathSafe.mockReturnValue(false);
      
      const result = ValidationService.validateFilePath('../malicious/path', 'File Path');
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('File Path contains invalid or unsafe path characters');
    });
  ...
  describe('validateFolderPath', () => {
    ...
    it('should validate folder paths', () => {
      const result = ValidationService.validateFolderPath('/valid/folder', 'Folder');
      expect(result.isValid).toBe(true);
    });
    
    it('should allow empty paths when not required', () => {
      const result = ValidationService.validateFolderPath('', 'Folder', { required: false });
      expect(result.isValid).toBe(true);
    });
  });
  ...
  describe('validateFileType', () => {
    it('should validate Excel files', () => {
      ...
      const result = ValidationService.validateFileType('test.xlsx', 'File', ['excel']);
      expect(result.isValid).toBe(true);
    });
    ...
    it('should validate multiple file types', () => {
      ...
      const result = ValidationService.validateFileType('test.csv', 'File', ['excel', 'csv']);
      expect(result.isValid).toBe(true);
    });
    ...
    it('should reject invalid file types', () => {
      ...
      const result = ValidationService.validateFileType('test.txt', 'File', ['excel']);
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('File must be one of these file types: excel');
    });
  });
  ...
  describe('validateUserSettings', () => {
    ...
    it('should validate concurrent downloads range', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Concurrent Downloads must be no more than 20');
    });
  ...
  describe('sanitizeString', () => {
    ...
    it('should replace spaces with underscores when requested', () => {
      const result = ValidationService.sanitizeString('hello world', { replaceSpaces: true });
      expect(result).toBe('hello_world');
    });
    
    it('should remove special characters when not allowed', () => {
      const result = ValidationService.sanitizeString('test@#$%', { allowSpecialChars: false });
      expect(result).toBe('test');
    });
    
    it('should truncate to max length', () => {
      const result = ValidationService.sanitizeString('verylongstring', { maxLength: 5 });
  ...
  describe('validateNoDuplicates', () => {
    it('should validate arrays without duplicates', () => {
      const result = ValidationService.validateNoDuplicates([1, 2, 3], 'Numbers');
      expect(result.isValid).toBe(true);
    });
    
    it('should reject arrays with duplicates', () => {
      const result = ValidationService.validateNoDuplicates([1, 2, 1], 'Numbers');
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('Numbers cannot contain duplicate values');
    });
    ...
    it('should reject non-arrays', () => {
      const result = ValidationService.validateNoDuplicates('not an array' as any, 'Field');
      expect(result.isValid).toBe(false);
