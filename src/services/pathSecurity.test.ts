...
describe('pathSecurity', () => {
  ...
  describe('sanitizePath', () => {
    ...
    it('should throw on null or invalid input', () => {
      expect(() => sanitizePath(null as any, testRoot)).toThrow(PathSecurityError);
      expect(() => sanitizePath('', testRoot)).toThrow(PathSecurityError);
      expect(() => sanitizePath(123 as any, testRoot)).toThrow(PathSecurityError);
    });

    it('should detect and prevent path traversal with ..', () => {
      expect(() => sanitizePath('../etc/passwd', testRoot)).toThrow(PathSecurityError);
      expect(() => sanitizePath('../../etc/passwd', testRoot)).toThrow(PathSecurityError);
      expect(() => sanitizePath('test/../../../etc/passwd', testRoot)).toThrow(PathSecurityError);
    });

    it('should detect null byte injection', () => {
      expect(() => sanitizePath('test.txt\0.jpg', testRoot)).toThrow(PathSecurityError);
      expect(() => sanitizePath('test\0/etc/passwd', testRoot)).toThrow(PathSecurityError);
    });

    it('should enforce allowed root directory', () => {
      expect(() => sanitizePath('/etc/passwd', testRoot)).toThrow(PathSecurityError);
      expect(() => sanitizePath(path.resolve('/other/path'), testRoot)).toThrow(PathSecurityError);
    });
    ...
    it('should handle multiple allowed roots', () => {
      ...
      expect(() => sanitizePath('test.txt', '/unsafe', options)).toThrow(PathSecurityError);
    });
  ...
  describe('validateFileAccess', () => {
    ...
    it('should return false for directories', async () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('not a file'));
    ...
    it('should return false for unsafe paths', async () => {
      ...
      expect(result).toBe(false);
      expect(consoleWarnSpy).toHaveBeenCalledWith(expect.stringContaining('File access denied'));
  ...
  describe('safeReadDir', () => {
    it('should read directory contents safely', async () => {
      ...
      mockedFs.readdir.mockResolvedValue(['file1.txt', 'file2.txt', 'subfolder'] as any);
    ...
    it('should throw for non-directory paths', async () => {
      ...
      await expect(safeReadDir(testFile, testRoot)).rejects.toThrow(PathSecurityError);
    });
    ...
    it('should skip unsafe entries', async () => {
      ...
      mockedFs.readdir.mockResolvedValue(['safe.txt', '..', 'test\0.txt'] as any);
    ...
    it('should throw PathSecurityError for unsafe directory path', async () => {
      await expect(safeReadDir('../etc', testRoot)).rejects.toThrow(PathSecurityError);
    });
    ...
    it('should wrap I/O errors in PathSecurityError', async () => {
      ...
      await expect(safeReadDir(testRoot)).rejects.toThrow('Failed to read directory');
    });
  });
