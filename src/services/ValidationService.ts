...
import { sanitizePath, isPathSafe, PathSecurityError } from './pathSecurity';
...
export class ValidationService {
   ...
   */
  static validateRequired(value: any, fieldName: string, options: ValidationOptions = {}): ValidationResult {
  ...
  static validateEmail(email: string, fieldName: string = 'Email'): ValidationResult {
    ...
    if (!emailRegex.test(trimmedEmail)) {
      return { isValid: false, errors: [`${fieldName} must be a valid email address`] };
    }
  ...
  static validateUrl(url: string, fieldName: string = 'URL'): ValidationResult {
    ...
    try {
      ...
      if (trimmedUrl.includes('://') && !trimmedUrl.startsWith('http://') && !trimmedUrl.startsWith('https://')) {
        return { isValid: false, errors: [`${fieldName} must be a valid URL or file path`] };
      }
   ...
   */
  static validateFilePath(filePath: string, fieldName: string, allowedRoot?: string): ValidationResult {
    try {
   ...
   */
  static validateFolderPath(folderPath: string, fieldName: string, options: ValidationOptions = {}): ValidationResult {
  ...
  static validateFileType(
    ...
    const isValidType = allowedTypes.some(type => typeCheckers[type](trimmedName));
  ...
  static validateDownloadConfig(config: DownloadConfig): ValidationResult {
    ...
    const partNoResult = this.validateRequired(config.partNoColumn, 'Part Number Column');
    ...
    if (config.imageColumns.length > 0) {
      const imageFolderResult = this.validateFolderPath(config.imageFolder, 'Image Download Folder');
      errors.push(...imageFolderResult.errors);
    }
    
    // PDF folder required if PDF column selected
    if (config.pdfColumn) {
      const pdfFolderResult = this.validateFolderPath(config.pdfFolder, 'PDF Download Folder');
      errors.push(...pdfFolderResult.errors);
    }
    
    // Validate worker count
    const workerResult = this.validateNumberRange(config.maxWorkers, 'Max Workers', 1, 20);
  ...
  static validateUserSettings(settings: UserSettings): ValidationResult {
    ...
    const pathsToCheck = [
      { path: settings.defaultPaths.imageDownloadFolder, name: 'Image Download Folder' },
      { path: settings.defaultPaths.pdfDownloadFolder, name: 'PDF Download Folder' },
      { path: settings.defaultPaths.sourceImageFolder, name: 'Source Image Folder' },
      { path: settings.defaultPaths.imageNetworkPath, name: 'Image Network Path' },
      { path: settings.defaultPaths.pdfNetworkPath, name: 'PDF Network Path' }
    ];
    
    for (const { path, name } of pathsToCheck) {
      if (path && path.trim()) {
        const pathResult = this.validateFolderPath(path, name, { required: false });
